...
Of course it would be nice if one could accept the call and actually talk to the other end. We're not quite there yet, but thanks to Srinivas Kandagatla who [submitted patches for the SLIMbus](https://lkml.org/lkml/2017/11/15/359) to mainline, the Linux kernel will soon be able to utilize the audio stack on Qualcomm phones like the Nexus 5. Then we can puzzle the userspace functionallity together.
