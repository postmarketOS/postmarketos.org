...
Of course it would be nice if one could accept the call and actually talk to the other end. We're not quite there yet, but thanks to [@Srinivas-Kandagatla](https://github.com/Srinivas-Kandagatla) who is working on mainline audio support for the [Snapdragon 820 SoC](https://en.wikipedia.org/wiki/List_of_Qualcomm_Snapdragon_devices#Snapdragon_820_and_821), the Linux kernel will soon be able to utilize the audio stack on Qualcomm phones like the Nexus 5. Then we can puzzle the userspace functionality together.
...
Speaking of free firmware: our recently established [#postmarketOS-lowlevel](/blog/2018/04/14/lowlevel/) crew is taking baby steps towards liberating bootloaders and cellular modem firmware for MediaTek devices. Around the time of the announcement blog post in April, quite a few people have joined the channel and started contributing. Notably [@cyrozap](https://github.com/cyrozap) has managed to dump the Boot ROMs (BROMs) of the MT6797 (Helio X20) and the MT6737M via [JTAG](https://en.wikipedia.org/wiki/JTAG). Together with the BROM of the MT6735P extracted by [@McBitter](https://github.com/McBitter), there's a lot of potential to understand the boot process with greater detail. After proper reverse engineering, we should be able to tell how the metadata is formatted on the flash memory and possibly find new options to recover from a bad flash. A better understanding of the firmware signature verification by the BROM may allow us to find vulnerabilities and finally bypass these restrictions.

Towards replacing components in the low level boot process on these SoCs, we also want to understand what state the BROM leaves the chip in after it's finished executing. Whether the BROM disables debugging functionallity, makes use of write-once registers or disables memory regions and registers. That information could be used to use U-Boot as Secondary Program Loader (SPL) for initialization in the future.
...
## No End In Sight
[![](/static/img/2018-06/wii-switch-thumb.png){: class="fr ml3 cr mb4"}](/static/img/2018-06/wii-switch.jpg)
